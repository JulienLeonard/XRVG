...
<svg width="%SIZE%" height="%SIZE%" %VIEWBOX% version="1.1" xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink">
  ...
  # shortcut method
  def fill(object, color = Color.black)
    add( object, Style[ :fill, color ] )
  end

  # shortcut method
  # if not width, width is object.length() / 1000.0
  def stroke(object, color = Color.black, strokewidth = nil)
    if not strokewidth
